...
import type { EvalTraceDetail, EvalTraceSummary, FaqRecord, FeedbackRecord } from '../../shared/types/admin'
...
export async function fetchEvalTrace(traceId: string): Promise<EvalTraceDetail> {
  return requestJson<EvalTraceDetail>({ method: 'GET', path: `/eval/trace/${encodeURIComponent(traceId)}` })
}

export async function fetchMetricsSummary(): Promise<MetricsSummaryItem[]> {
  const response = await requestJson<{ items: MetricsSummaryItem[] }>({ method: 'GET', path: '/eval/metrics/summary' })
  return response.items ?? []
}
...
export async function fetchEvalSessions(limit = 100): Promise<Array<{ session_id: string; trace_count: number; last_active?: string }>> {
  const response = await requestJson<{ items: Array<{ session_id: string; trace_count: number; last_active?: string }> }>({
    method: 'GET',
    path: '/eval/sessions',
    query: { limit },
  })
...
export async function fetchFaqs(adminKey: string, limit = 200, skip = 0): Promise<FaqRecord[]> {
...
export async function deleteFaq(adminKey: string, question: string): Promise<{ status: string; message?: string }> {
  return requestJson({
    method: 'DELETE',
    path: '/agent/admin/faqs',
    query: { question },
  ...
  await streamSse(
    ...
    },
    {
      onEvent: (eventName, data) => {
        if (eventName === 'error') {
          throw new Error(data || 'FAQ ingest failed')
        }
        if (data) {
          lastMessage = data
        }
...
export async function fetchModels(): Promise<Array<{ name: string; models: Array<{ id: string; name: string; provider?: string }> }>> {
  const response = await requestJson<{
    categories: Array<{ name: string; models: Array<{ id: string; name: string; provider?: string }> }>
...
export async function fetchSessionConfig(sessionId: string): Promise<{
  ...
  return requestJson({ method: 'GET', path: `/agent/config/${encodeURIComponent(sessionId)}` })
}
...
export async function fetchGuardrailEvents(adminKey: string, limit = 100): Promise<GuardrailEvent[]> {
...
export async function fetchUserAnalytics(adminKey: string, limit = 100): Promise<UserAnalyticsRow[]> {
...
export async function fetchConversations(adminKey: string, limit = 100): Promise<EvalTraceSummary[]> {
  const response = await requestJson<{ items: EvalTraceSummary[] }>({
    method: 'GET',
...
export async function listFeedback(adminKey: string, limit = 100): Promise<FeedbackRecord[]> {
