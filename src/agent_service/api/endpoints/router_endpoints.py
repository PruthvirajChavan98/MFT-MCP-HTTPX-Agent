...
def _resolve_tools_for_request(req: RouterClassifyRequest):
    if not req.session_id:
        return None
    sid = valid_session_id(req.session_id)
    return mcp_manager.rebuild_tools_for_user(sid, openrouter_api_key=req.openrouter_api_key)
...
async def router_classify(req: RouterClassifyRequest):
    ...
    try:
        tools = _resolve_tools_for_request(req)
