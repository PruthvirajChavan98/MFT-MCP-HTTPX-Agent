...
def _rows(q: str, params: dict) -> List[dict]:
    ...
    with driver.session() as session: return [dict(r) for r in session.run(q, **params)] # type: ignore
...
def _compress_events(events: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for evt in events:
        ...
        if current_block and is_token and current_block.get("event_type") == etype:
            ...
            if current_block.get("text"): current_block["text"] += new_text
            else: current_block["text"] = new_text
            continue
        if is_tool and compressed:
            last = compressed[-1]
            if last.get("event_type") == etype and last.get("name") == ename: continue
        if current_block:
        ...
        if is_token:
            ...
            compressed.append(e)
    if current_block: compressed.append(current_block)
    return compressed
...
async def eval_search(
    ...
    session_id: Optional[str] = None, status: Optional[str] = None, provider: Optional[str] = None,
    model: Optional[str] = None, case_id: Optional[str] = None,
    metric_name: Optional[str] = None, passed: Optional[bool] = None,
    min_score: Optional[float] = None, max_score: Optional[float] = None,
    order: Literal["desc", "asc"] = "desc",
):
    order_dir = "DESC" if order.lower() == "desc" else "ASC"
    if metric_name: metric_name = metric_name.strip()
    
    where = []
    params = {"limit": limit, "offset": offset, "session_id": session_id, "status": status, "provider": provider, "model": model, "case_id": case_id, "metric_name": metric_name, "passed": passed, "min_score": min_score, "max_score": max_score}
    ...
    metric_filters = []
    if metric_name: metric_filters.append("x.metric_name = $metric_name")
    if passed is not None: metric_filters.append("x.passed = $passed")
    if min_score is not None: metric_filters.append("x.score >= $min_score")
    if max_score is not None: metric_filters.append("x.score <= $max_score")
    ...
    if metric_filters:
        ...
        metric_clause = "OPTIONAL MATCH (t)-[:HAS_EVAL]->(mr:EvalResult) WITH t, collect(mr) AS evals"

    q_count = f"MATCH (t:EvalTrace) WHERE {' AND '.join(where)} {metric_clause} RETURN count(t) AS total"
...
# -----------------------------
@router.get("/sessions")
async def eval_sessions(limit: int = Query(25), offset: int = Query(0), app_id: Optional[str] = None):
    where_clause = ""
...
async def eval_trace(trace_id: str):
    ...
    if not row: raise HTTPException(status_code=404, detail="Trace not found")
    trace_obj = row["trace"]
    for k in ("inputs_json", "tags_json", "meta_json"): trace_obj[k] = _json_load_maybe(trace_obj.get(k))
...
# -----------------------------
@router.get("/fulltext")
async def eval_fulltext(q: str = Query(..., min_length=1), kind: Literal["event", "trace", "result"] = "event", limit: int = 50, offset: int = 0, trace_id: Optional[str] = None):
    index = {"event": "evalevent_text", "trace": "evaltrace_text", "result": "evalresult_text"}[kind]
    cypher = f"CALL db.index.fulltext.queryNodes($index, $q, {{skip: $offset, limit: $limit}}) YIELD node, score WHERE ($trace_id IS NULL OR node.trace_id = $trace_id) RETURN labels(node) AS labels, score AS score, node.trace_id AS trace_id, node.event_key AS event_key, node.seq AS seq, node.eval_id AS eval_id, node.metric_name AS metric_name, coalesce(node.text, node.final_output, node.reasoning) AS preview ORDER BY score DESC"
    rows = await run_in_threadpool(_rows, cypher, {"index": index, "q": q, "limit": limit, "offset": offset, "trace_id": trace_id})
