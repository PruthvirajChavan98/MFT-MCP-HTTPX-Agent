...
#         # Format history as string list for Judge
#         self.chat_history = []
#         if chat_history:
...
#         # Capture ALL tool outputs for context
...
#             "meta": {
#                 "system_prompt": self.system_prompt[:500], # Store snippet for debug
...
#     tool_names: Set[str],
# ) -> List[Dict[str, Any]]:
#     # ... (Same regex/deterministic logic as before) ...
...
# # --- NEW: LLM Metrics Wrapper with FULL CONTEXT ---
# async def compute_llm_metrics(
#     trace: Dict[str, Any],
#     collector: ShadowEvalCollector # Pass full collector to get sys prompt etc
# ) -> List[Dict[str, Any]]:
...
#     # 1. Prepare Context Blob
#     tool_outputs = "\n\n".join(collector.retrieved_context) if collector.retrieved_context else "No tool outputs."
#     history_str = "\n".join(collector.retrieved_context) if collector.chat_history else "No history."
...
#         # Pass collector to get extra context
...
from src.agent_service.eval_store.neo4j_store import EvalNeo4jStore
from src.agent_service.eval_store.judge import judge_service
from src.agent_service.eval_store.embedder import EvalEmbedder  # ✅ NEW IMPORT
from src.agent_service.core.config import ENABLE_LLM_JUDGE, JUDGE_MODEL_NAME
from src.agent_service.llm.client import get_llm

log = logging.getLogger("shadow_eval")
STORE = EvalNeo4jStore()
EMBEDDER = EvalEmbedder()  # ✅ NEW INSTANCE
...
async def maybe_shadow_eval_commit(collector: ShadowEvalCollector) -> None:
    try:
        ...
        # ✅ NEW: Trigger Embeddings for Vector Search
        try:
            ...
            # Optionally embed results too (not strictly needed for basic search, but good for completeness)
            for ev in evals:
