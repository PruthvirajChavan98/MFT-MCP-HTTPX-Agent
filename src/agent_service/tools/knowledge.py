...
class KnowledgeBaseService:
    ...
    async def _get_metadata_chain(self, custom_key: Optional[str] = None):
        ...
        prompt = ChatPromptTemplate.from_messages([
            ("system", "You are a Data Engineer. Extract topics, products, and contact entities.\n{format_instructions}"),
            ("human", "{text}")
        ])
    ...
    async def ingest_faq_batch_gen(self, items: List[dict], groq_key: str = None, openrouter_key: str = None):
        ...
        yield {"event": "progress", "data": json.dumps({"percent": 5, "message": "Initializing..."})}
    ...
    async def edit_faq(self, original_question: str, new_question: str = None, new_answer: str = None, openrouter_key: str = None) -> dict:
        if not original_question: return {"status": "error", "message": "Original question required"}
        ...
        if new_question and new_question != original_question:
            if not openrouter_key:
                return {"status": "error", "message": "OpenRouter Key required to update question text (re-embedding needed)."}
            try:
                embeddings = get_embeddings(api_key=openrouter_key)
    ...
    async def delete_faq(self, question: str) -> dict:
        try:
            Neo4jManager.execute_write("MATCH (q:Question {text: $q}) DETACH DELETE q", {"q": question})
    ...
    async def get_metadata_tags(self) -> dict:
        ...
        try:
            ...
            if not result: return {"topics": [], "products": []}
            row = result[0]
            return {
    ...
    async def delete_faq_by_vector(self, query: str, threshold: float = 0.92, openrouter_key: str = None) -> dict:
        if not openrouter_key: return {"status": "error", "message": "Key required"}
        try:
            embeddings = get_embeddings(api_key=openrouter_key)
            vector = await embeddings.aembed_query(query)
