...
def _build_trace_doc(trace: Dict[str, Any], events: List[Dict[str, Any]]) -> str:
    ...
    if trace.get("session_id"): lines.append(f"Session ID: {trace.get('session_id')}")
    if trace.get("case_id"): lines.append(f"App/Case ID: {trace.get('case_id')}")
    if trace.get("error"): lines.append(f"Error: {trace.get('error')}")
