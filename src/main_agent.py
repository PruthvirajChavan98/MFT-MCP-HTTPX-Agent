...
from src.agent_service.core.schemas import (
    AgentRequest, SessionConfig, RouterClassifyRequest
)
from src.agent_service.core.utils import valid_session_id, _extract_tool_output
from src.agent_service.core.cost import calculate_run_cost, calculate_run_cost_detailed
...
async def get_session_config(session_id: str):
    ...
    return {
        ...
        "has_custom_key": has_or_key or has_nv_key or has_groq_key,
...
async def _resolve_agent_resources(sid: str, request: AgentRequest):
    ...
    provider = request.provider or saved_config.get("provider")
    
    # Priority: Request > Saved Config > None
    ...
    # Unified Factory Init with explicit provider
    model = get_llm(
    ...
    # Return 6 values including provider
    return model, tools, sys_prompt, or_key, model_name, provider
...
async def query_agent(request: AgentRequest):
    try:
        ...
        model, tools, sys_prompt, or_key, _, provider = await _resolve_agent_resources(sid, request)
        ...
        return {"response": resp, "router": router_out, "provider": provider}
    except Exception as e:
...
async def stream_agent(request: AgentRequest):
    try:
        sid = valid_session_id(request.session_id)
        model, tools, sys_prompt, or_key, model_id, provider = await _resolve_agent_resources(sid, request)
        ...
        if kb_payload:
            async def kb_event_generator():
                try:
                    ...
                    # No cost for KB-first (cached response)
                    yield {"event": "cost", "data": json.dumps({
                        "total_cost": 0.0,
                ...
                async for event in agent.astream_events(
                    ...
                    # Enhanced Cost Tracking
                        ...
                        if hasattr(out_msg, "usage_metadata") and out_msg.usage_metadata:
                            ...
                            cost, breakdown = await calculate_run_cost_detailed(model_id, usage, provider) # type: ignore
                            total_cost += cost
                            
                            # Accumulate tokens
                            cumulative_usage["prompt_tokens"] += usage.get("input_tokens", 0) or usage.get("prompt_tokens", 0)
                            cumulative_usage["completion_tokens"] += usage.get("output_tokens", 0) or usage.get("completion_tokens", 0)
                            cumulative_usage["total_tokens"] += usage.get("total_tokens", 0)
                            
                            if usage.get("reasoning_tokens"):
                                cumulative_usage["reasoning_tokens"] += usage.get("reasoning_tokens", 0)
                            if usage.get("cache_read_input_tokens"):
                                cumulative_usage["cached_tokens"] += usage.get("cache_read_input_tokens", 0)

                collector.on_done(final_output="", error=None)
                
                # Final detailed cost event
                yield {"event": "cost", "data": json.dumps({
                    "total_cost": round(total_cost, 6),
                    "usage": {k: v for k, v in cumulative_usage.items() if v > 0},
                    "model": model_id,
                    "provider": provider,
                    "currency": "USD"
                }, ensure_ascii=False)}
...
async def generate_follow_up(request: AgentRequest):
    try:
        sid = valid_session_id(request.session_id)
        model, tools, _, or_key, _, provider = await _resolve_agent_resources(sid, request)
