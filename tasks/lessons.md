...
### 24. Docker Compose interpolates `$` in env_file values — argon2 hashes must be `$$`-escaped
**Trigger:** During the 2026-04-17 admin-auth cutover, enrollment produced an argon2 hash like `$argon2id$v=19$m=65536,t=3,p=4$SALT$HASH`. Pasted verbatim into the repo-root `.env`. Container booted clean, but every login returned `{"code": "invalid_credentials"}`. Root cause: Docker Compose processes `env_file` values as shell-like variable substitutions. Each `$argon2id`, `$v`, `$m`, `$SALT`, `$HASH` resolved to empty string. `docker exec <container> printenv SUPER_ADMIN_PASSWORD_HASH` showed `=19=65536,t=3,p=4/SALT+/HASH` (stripped).
**Rule:** Any `$` in an `env_file` value that must survive to the container verbatim MUST be doubled to `$$`. In practice this bites exactly two shapes: argon2/bcrypt password hashes, and any secret that happens to start with `$`. Apply at `.env`-write time (enrollment runbook step 2) AND document prominently in `.cursor/rules/admin-auth.mdc`. The enrollment script should probably emit `$$`-escaped output in a future iteration so this trap stops catching operators. Verify fix via `docker exec <container> printenv <VAR>` — the resolved value must match the intended plaintext.
