...
const withAdminHeadersMock = vi.fn((key?: string, extra?: Record<string, string>) => ({
  'X-Admin-Key': key ?? '',
  ...(extra ?? {}),
}))
...
describe('admin faq api contract', () => {
  it('fetchFaqs uses legacy stable endpoint', async () => {
    ...
    const rows = await fetchFaqs('admin-key', 100, 0)
  ...
  it('ingestFaqBatch streams through stable batch-json endpoint', async () => {
    streamSseMock.mockImplementation(async (_url: string, _init: RequestInit, handlers: { onEvent: (eventName: string, data: string) => void }) => {
    ...
    await ingestFaqBatch('admin-key', [{ question: 'Q', answer: 'A' }], (msg) => progress.push(msg))
  ...
  it('ingestFaqPdf posts multipart stream to upload-pdf endpoint', async () => {
    ...
    await ingestFaqPdf('admin-key', file)

    const call = streamSseMock.mock.calls.find(([url]: [string]) => url.endsWith('/agent/admin/faqs/upload-pdf'))
  ...
  it('ingestFaqBatch surfaces structured SSE error payload message', async () => {
    streamSseMock.mockImplementation(async (_url: string, _init: RequestInit, handlers: { onEvent: (eventName: string, data: string, parsed?: unknown) => void }) => {
      handlers.onEvent(
        'error',
    ...
    await expect(
      ingestFaqBatch('admin-key', [{ question: 'Q', answer: 'A' }]),
    ).rejects.toThrow('Batch FAQ ingest failed.')
  })
  ...
  it('fetchFaqCategories uses additive managed endpoint', async () => {
    ...
    const rows = await fetchFaqCategories('admin-key')
  ...
  it('deleteFaq supports id query for stable row identity', async () => {
    ...
    await deleteFaq('admin-key', { id: 'faq-123' })
  ...
  it('searchFaqSemantic posts semantic query body to existing endpoint', async () => {
    ...
    const rows = await searchFaqSemantic('admin-key', 'loan', 5)
