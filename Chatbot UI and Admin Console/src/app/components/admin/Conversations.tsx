...
import { Search, MessageSquare, Database, Microscope } from 'lucide-react'
import { fetchConversations, fetchEvalSessions, fetchSessionTraces } from '../../../shared/api/admin'
...
export function Conversations() {
  ...
  const [search, setSearch] = useState(initialSessionId || '')
  ...
  const { data: sessionTraces = [], isLoading: sessionLoading } = useQuery({
    ...
    enabled: !!auth.adminKey && !!selectedSessionId,
  })

  const filtered = data.filter((c: SessionListItem) =>
    !search ||
    (c.first_question || '').toLowerCase().includes(search.toLowerCase()) ||
    c.session_id.includes(search)
  )

  const selected = filtered.find(c => c.session_id === selectedSessionId) || (selectedSessionId ? { session_id: selectedSessionId, started_at: sessionTraces.length ? new Date(sessionTraces[0].timestamp).toISOString() : undefined, model: 'Agent Session' } : null)
  ...
  // 🚀 ENTERPRISE PRODUCTION-GRADE SOLUTION:
  // The API natively streams complete ChatMessage structures from the LangGraph Redis checkpointer.
  // No hacky mapping patchwork is required.
    ...
    <div className="space-y-6 max-w-[1600px] mx-auto pb-10">
      ...
      <Tabs defaultValue="live" className="w-full">
                    ...
                    <div className="p-3 space-y-1.5">
                      {filtered.map((c) => (
                        <button
                          ...
                          className={`w-full text-left p-4 rounded-xl border transition-all duration-200 ${selectedSessionId === c.session_id
        ...
        <TabsContent value="evals" className="m-0 border-none outline-none">
          <Card className="border-gray-200 shadow-md overflow-hidden bg-white" style={{ height: 'calc(100vh - 220px)', minHeight: 500 }}>
                  ...
                  <tbody className="divide-y divide-gray-50/50">
                    {evalLoading ? (
                      Array.from({ length: 5 }).map((_, i) => (
                        <tr key={i}><td colSpan={3} className="px-6 py-4"><Skeleton className="w-full h-8" /></td></tr>
                      ))
                    ) : evalSessions.map((s) => (
                      <tr key={s.session_id} className="hover:bg-slate-50/50 transition-colors">
                        ...
                        <td className="px-6 py-3 text-right text-slate-500 text-xs">{s.last_active ? formatDateTime(s.last_active) : '—'}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
