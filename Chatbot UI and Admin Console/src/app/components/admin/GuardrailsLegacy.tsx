...
export function GuardrailsLegacy() {
  ...
  const decisions = useMemo(
    () => ['all', ...new Set(data.map((event) => event.risk_decision).filter(Boolean))],
    [data],
  )
  const trendPeak = useMemo(() => Math.max(...trends.map((item) => item.total_events), 1), [trends])
  const summaryErrorMessage = (summaryError as Error | undefined)?.message
  const queueErrorMessage = (queueError as Error | undefined)?.message
  const judgeErrorMessage = (judgeError as Error | undefined)?.message
  const trendsErrorMessage = (trendsError as Error | undefined)?.message
  const eventsErrorMessage = (error as Error | undefined)?.message
  const kpiErrorMessages = [summaryErrorMessage, queueErrorMessage, judgeErrorMessage].filter(
  ...
  return (
    <div className="space-y-6">
      ...
      {!!kpiErrorMessages.length && (
        <Alert variant="destructive">
          <AlertDescription>
            KPI data partially unavailable: {kpiErrorMessages.join(' · ')}
          </AlertDescription>
        </Alert>
      )}
        ...
        <Card>
          <CardContent className="pt-5 space-y-3">
            ...
            {trendsLoading ? (
              ...
              <Alert variant="destructive">
                <AlertDescription>Trend data unavailable: {trendsErrorMessage}</AlertDescription>
              </Alert>
            ) : (
      ...
      <Card>
        <CardContent className="p-0">
          {isLoading ? <div className="p-4 space-y-2">{Array.from({ length: 8 }).map((_, i) => <Skeleton key={i} className="h-12 rounded" />)}</div> : eventsErrorMessage ? (
            <div className="p-4">
              <Alert variant="destructive">
                <AlertDescription>{eventsErrorMessage}</AlertDescription>
              </Alert>
            </div>
          ) : (
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead className="bg-slate-50 border-b">
                  <tr>{['Time', 'Session', 'Risk', 'Decision', 'Path', 'Reasons', 'Action'].map((h) => <th key={h} className="px-4 py-2.5 text-left text-xs font-semibold text-muted-foreground uppercase tracking-wide">{h}</th>)}</tr>
                </thead>
                <tbody>
                  {data.map((ev, i) => (
                    <tr key={i} className="border-b last:border-0 hover:bg-slate-50/50 cursor-pointer" onClick={() => setSelected(ev)}>
                      ...
                      <td className="px-4 py-2.5 text-xs">
                        {buildConversationHref(ev.session_id) ? (
                          <Link
                            to={buildConversationHref(ev.session_id)!}
                            onClick={(event) => event.stopPropagation()}
                            className="inline-flex items-center rounded-md border border-cyan-200 bg-cyan-50 px-2 py-1 font-semibold text-cyan-700 transition hover:bg-cyan-100"
                          >
                            View Conversation
                          </Link>
                        ) : (
                  ...
                  {!data.length && <tr><td colSpan={7} className="px-4 py-8 text-center text-sm text-muted-foreground">No events</td></tr>}
                </tbody>
              </table>
            </div>
          )}
        ...
        <SheetContent>
          ...
          {selected && (
            <div className="mt-4 space-y-3 text-sm">
              ...
              <div>
                {buildConversationHref(selected.session_id) ? (
                  <Link
                    to={buildConversationHref(selected.session_id)!}
                  ...
                  <span className="text-xs text-muted-foreground">Conversation unavailable for this event.</span>
                )}
              </div>
            </div>
          )}
